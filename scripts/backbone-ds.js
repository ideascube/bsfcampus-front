(function(window, undefined) {
	...
	var factory = function(Backbone) {
		 ...
		 */
		DS.isIncomplete = function(resourceName, id) {
		...
		/**
		 * Set a model as complete or not
		 * @param  {String} resourceName 	The name of the resource when defined
		 * @param  {Number|String} id     The unique ID of the model to find
         * @param  {Boolean} isComplete     Wether the resource is complete or not
		 */
		DS.setComplete = function(resourceName, id, isComplete) {
			if (isComplete && DS.isIncomplete(resourceName, id)) {
        ...
        /**
         * Synchronously return a collection with models filtered by ids
         * @param {String} resourceName     The name of the resource when defined
         * @param {Number|String} ids       A list of the object ids to get
         * @return {Collection}             A new filtered resource collection
         */
        DS.getMany = function(resourceName, ids) {
            ...
            for (var i=0; i < ids.length; i++) {
                var model = DS.get(resourceName, ids[i]);
                if (model) {
		...
		function findCollectionResource(resourceName, id) {
			...
			if (model) {
				if (DS.isIncomplete(resourceName, id)) {
